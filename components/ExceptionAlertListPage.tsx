...
interface ExceptionAlertListPageProps {
...
export default function ExceptionAlertListPage({
  alerts,
  onBack,
  onAssignTask,
  onVerify
}: ExceptionAlertListPageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 sticky top-0 z-10">
        ...
        <h1 className="font-bold text-gray-800 text-sm flex-1">异常预警列表</h1>
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-4">
        {activeAlerts.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-64 text-gray-400">
            ...
            <p className="text-sm font-medium">暂无异常预警</p>
            <p className="text-xs mt-1">系统正在实时监控门店运营状态</p>
          </div>
        ) : (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-red-500 font-medium">{activeAlerts.length} 项待处理</span>
              </div>
            </div>
